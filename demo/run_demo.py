...
def pickTwoDifferent():
    algorithmList = ["FM^3 (OGDF)", "Fast Multipole Embedder (OGDF)",
                     "Fast Multipole Multilevel Embedder (OGDF)",
                     "Pivot MDS (OGDF)", "GRIP"]
    string1 = random.choice(algorithmList);
    string2 = random.choice(algorithmList);
    if string1 == string2:
        pickTwoDifferent()
...
print(pickTwoDifferent()[0])

def runLayout(graphID, algorithm):
    with open('../backend/test_data/%s_links.json' % graphID) as f:
...
def writeLayout(nodeDict, graphID):
    with open('../backend/test_data/%s_nodes_viz.json' % graphID) as f:
    ...
    with open('../backend/test_data/%s_clus_viz.json' % graphID, 'r') as f:
