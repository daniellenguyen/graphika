...
def writeLayout(nodeDict, graphID):
    ...
    # with open('../docs/jsons/%s_%s.json' % (graphID, algo), 'w') as f:
    #     f.write(json.dumps(output))
...
if __name__ == '__main__':
    ...
    # graphIDs = 'actor_0','actor_1','actor_2','actor_3','actor_4','astro','comics','gnutella','Goodreads','highEnergy','USAir'
    # algos = 'FM^3 (OGDF)','Pivot MDS (OGDF)','Fast Multipole Embedder (OGDF)','Fast Multipole Multilevel Embedder (OGDF)','GRIP'

    # for graph in graphIDs:
    #     for algo in algos:
    #         print(graph, algo)
    #         coords = runLayout(graph, algo)
    #         writeLayout(coords, graph, algo)
