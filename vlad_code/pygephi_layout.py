...
class GephiVisualizer(object):
    ...
    def __init__(self, layout_options_file_name):
        ...
        self.published_map_id = self.layout_options["published_map_id"]
    ...
    def getGraphFromClustermap(self):
        ...
        clumap_id_file = open("%s/%d_node_arcs.json" % (self.layout_path, self.published_map_id),"r")
    ...
    def rotateGraph(self):
        ...
        for graph_angle in range(0, 360.0, 45.0):
            #rotate by 45d every time so we don't over-rotate
            graph_layout.angle = min(graph_angle, 45.0)
            total_steps = 100
            self.runLayout(graph_layout, layout_type, total_steps)
            graph_layout_quality_hash[self.evaluateRotation(graph)] = graph_angle
